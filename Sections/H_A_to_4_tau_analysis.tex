...
\begin{enumerate}[i)]
  \item A model-independent search for the $Z^{*}\rightarrow \phi A \rightarrow 4\tau$ process. Both additional particles are required to have narrow widths. No assumptions are made on the production cross-section or the branching fractions.
...
The branching fractions of $\phi$ and A are calculated using \textsc{2HDECAY}~\cite{Krause:2018wmo}.
...
Similarly to Section~\ref{sec:high_mass_optimisation}, events in each of the 10 analysis channels and categories are drawn out into a histogram based on the same discriminating variable, the total transverse mass ($m_{T}^{\text{tot
}}$).
...
In addition, tri-boson samples are used that are generated with \MGvATNLO at \ac{NLO} precision~\cite{Alwall:2011uj}.
...
The cross-sections for this analysis are scaled to higher-order precisions than generated using K factors~\cite{}.
...
The process binning used is also an admission that the relevant parametrisation of the $\FF$ between processes is too complicated to model with \say{bin and fit} methods.
...
Reference~\cite{Rogozhnikov:2016bdp} proposes a new method for reweighting utilising \ac{ML} techniques to solve the issues of dimensionality. 
...
\begin{figure}[p!]
\centering
    \subfloat[]{\includegraphics[width=0.5\textwidth]{Figures/subtraction_plot_pt_1_mttt_pass.pdf}}
    \subfloat[]{\includegraphics[width=0.5\textwidth]{Figures/subtraction_plot_jet_pt_1_divide_pt_1_mttt_pass.pdf}} \\
    \subfloat[]{\includegraphics[width=0.5\textwidth]{Figures/subtraction_plot_tau_decay_mode_1_mttt_pass.pdf}}
    \subfloat[]{\includegraphics[width=0.5\textwidth]{Figures/subtraction_plot_eta_1_mttt_pass.pdf}}
...
\begin{figure}[p!]
\centering
    \subfloat[]{\includegraphics[width=0.5\textwidth]{Figures/reweight_ave_plot_pt_1_all_mttt_all_years.pdf}}
    \subfloat[]{\includegraphics[width=0.5\textwidth]{Figures/reweight_ave_plot_jet_pt_1_divide_pt_1_all_mttt_all_years.pdf}} \\
    \subfloat[]{\includegraphics[width=0.5\textwidth]{Figures/reweight_ave_plot_tau_decay_mode_1_all_mttt_all_years.pdf}}
    \subfloat[]{\includegraphics[width=0.5\textwidth]{Figures/reweight_ave_plot_eta_1_all_mttt_all_years.pdf}}
...
\begin{figure}[!hbtp]
\centering
    \subfloat[]{\includegraphics[width=0.5\textwidth]{Figures/closure_plot_pt_1_all_mttt_all_years_all.pdf}}
    \subfloat[]{\includegraphics[width=0.5\textwidth]{Figures/closure_plot_jet_pt_1_divide_pt_1_all_mttt_all_years_all.pdf}} \\
    \subfloat[]{\includegraphics[width=0.5\textwidth]{Figures/closure_plot_tau_decay_mode_1_all_mttt_all_years_all.pdf}}
    \subfloat[]{\includegraphics[width=0.5\textwidth]{Figures/closure_plot_eta_1_all_mttt_all_years_all.pdf}}
...
The removal of the non \jtth backgrounds from the fitting region is covered by two sources of uncertainties.
...
The MC is shifted up and down by 10\% and the \ac{BDT} subtraction method and the reweighting are repeated with the differing datasets. \\
...
There are no upward deviations of the number of events observed, that are with any mass hypotheses for the signal model searched for.
...
This is not present in the $m_{\phi}=100$ GeV scenario as the $\phi$ boson is not heavy enough.
The limit then flattens for the remainder of the $m_{A}$ phase space shown, as the $\PH\rightarrow Z\PA$ and $\PA\rightarrow Z\Ph$ decays only minimally hinder the $\tau\tau$ branching fractions of $\phi$ and $\PA$. \\
