...
\begin{enumerate}[i)]
  \item A model independent search for single narrow spin-0 resonance ($\phi$), produced via gluon fusion (gg$\phi$) or in association with a b quark (bb$\phi$). The \ac{SM} Higgs boson is treated as a background and the Yukawa couplings of the spin-0 resonance that contribute to the gluon fusion loop are set to \ac{SM} values.
   \item A search for the \ac{MSSM} Higgs sector in a number of benchmark scenarios. The benchmark scenarios were proposed in References~\cite{Bahl:2018zmf,Bahl:2020kwe,Bahl:2019ago} and summarised in Reference~\cite{Bagnaschi:2791954}. The $M_{h}^{125}$ and $M_{h,\text{ EFT}}^{125}$ scenarios are shown in this chapter. The production of the observed Higgs boson particle at 125 GeV is also used to constrain the available phase space.
...
With the $\tan\beta$ enhancement, the decays of additional Higgs bosons to $\tau$ leptons and b quarks are most likely.
$\tau$ leptons are identified with a higher purity than bottom quarks at the \ac{CMS} detector.
...
This hypothesis was tested with the 2016 dataset and although no deviations were observed, the strongest limits on the \ac{MSSM} phase space was placed by the $\tau^+\tau^-$ final states \cite{CMS_MSSM_Tau_2018,Khachatryan:2015tra}. \\
...
To account for the different loop contributions at the \ac{NLO} plus parton shower prediction, weights based off the $\pT$ spectra are calculated to split the contributions from the t quark only, b quark only, and tb-interference.
...
Further studies have been used to search for single and pair production at the \ac{CMS} experiment and no statistically significance derivation was observed~\cite{CMS:2022zks}.
...
The variable is used to remove W + jets background events, where a jet is misidentified as a $\tau_h$ and the \ac{MET} and light lepton from the W decay are aligned and hence the event has a large $m_{T}(\pTvec^{\hspace{2pt}e/\mu},\pTvec^{\hspace{2pt}\text{MET}})$. \\
...
To resolve this issue a likelihood-based method named \texttt{SVFit} algorithm is used \cite{Bianchini:2014vza}.
...
Each event is initially split into categories depending on whether it has 0 or $\geq$1 b tagged jets in the event.
...
In the $\emu$ channel, is subdivided into three signal categories based of the cuts on the variable $D_{\zeta}$ as defined in Equation~\ref{eqn:Dzeta_def}.
...
When compared with data, W + jets, $Z\rightarrow ll$, $\ttbar$, and single top quark events in the tW channel are normalised to their cross sections at \ac{NNLO} precision\cite{Melnikov:2006kv,Czakon:2011xx,Kidonakis:2013zqa}, while single top quark (t-channel) and diboson events are normalized to their cross sections at \ac{NLO} precision or higher~\cite{Kidonakis:2013zqa,Campbell:2011bn,Gehrmann:2014fva}.

\section{QCD Estimation in the $\emu$ Channel}
\label{sec:qcd}

The \ac{QCD} model in the $\emu$ channel, that attempts to model events where two jets are misidentified as an election muon pair, is taken from data with a same sign pair with a transfer factor ($F_{T}$).
...
One such procedure is the $\FF$ method. \\
...
This is defined as the signal region but with the criteria that the jet$\rightarrow\tauh$ events fail the nominal $\tau_h$ ID but pass the looser alternative $\tau_h$ ID requirement.
...
The \ac{QCD} process is assumed to produce two jet fakes and so the $\FF$ is chosen to be calculated from leading $\pT$ $\tau_h$ candidate only. 
...
These alternative $\FF$ are applied to the fail region in D and corrections are calculated comparing it to the pass region in D.
...
The fits are flattened at $\pT^{\tau_h}$ values where there is no significant downwards shift or at the final bin. 
...
\label{eqn:c_w}
\end{equation}
When either correction variable is separated from 0, a larger quantity of fake \ac{MET} is expected in the event. 
