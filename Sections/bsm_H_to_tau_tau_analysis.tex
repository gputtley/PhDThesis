...
\end{subfigure}
\vspace*{10mm}
\caption{Diagram (a) shows the production of neutral Higgs bosons from gluon fusion. The dominant loop contributions to these diagrams are from t-only, b-only and tb-interference. Diagrams (b) and (c) show production in association with b quarks.}
...
Further contributions from any supersymmetric partners were checked and accounted for less than a few \% and so are neglected.
...
The value of the mass is allowed to vary within this window, however, the Yukawa couplings are rescaled to the observed mass.
...
\end{subfigure}
\vspace*{10mm}
\caption{Feynman diagrams showing the contribution from $U_1$ vector leptoquarks to the final state with a pair of oppositely charged $\tau$ leptons. $q$ represents any SM quark. The t-channel (a), pair (b) and single (c) production of a vector leptoquark is shown.}
...
A search for single and pair production at the \ac{CMS} experiment was performed and no statistically significant derivation was observed~\cite{CMS:2022zks}.
...
The t-channel process contains two vertices with a $U_1$ vector leptoquark, a quark and a $\tau$ lepton, and hence the cross-section will scale with $g_{U}^4$.
...
The signal process of a $U_1$ t-channel exchange is simulated in the five \ac{FS} at \ac{LO} precision using the \MGvATNLO v2.6.5 event generator~\cite{Alwall:2011uj}.
...
The interference between the $U_1$ signal and $Z/\gamma^* \rightarrow \tau\tau$ production was checked and a large destructive effect was observed, with magnitude dependent on $g_{U}$.
...
\begin{table}[hbtp]
  ...
  \begin{tabular}{|c||c|c|c|c|}
    ...
    \hline        
  \end{tabular}
  \caption{Lower trigger light lepton thresholds $\pT$ in GeV for the $\etauh$ and $\mutauh$ channels.}
...
In these events the \ac{MET} (neutrino) and light lepton from the W decay are aligned and hence the event has a large $m_{T}(\pTvec^{\hspace{2pt}e/\mu},\pTvec^{\hspace{2pt}\text{MET}})$. \\
...
\begin{enumerate}[i)]
  ...
  \item Events with jets faking both light leptons ($\text{jet}\rightarrow l$) in the $\emu$ channel.
...
Once completed, the selected muons and their kinematic properties are replaced with a simulated $\tau$ lepton.
...
Firstly, the determination regions are defined, which are \jtth enriched control regions orthogonal to the signal region. 
They are then purified by subtracting off any non \jtth events with \ac{MC}.
It is used to calculate a $\FF$ by taking the ratio of the number of \jtth events that pass the nominal $\tau_h$ identification requirement ($N(\texttt{Nominal})$), to the number of jet fake events that fail the nominal $\tau_h$ identification but pass a looser alternative $\tau_h$ identification requirement ($N(\texttt{Alternative}\text{ and not }\texttt{Nominal})$), as shown in Equation~\ref{eqn:ff}.
...
This is defined as the signal region but with the criteria that the \jtth events fail the nominal $\tau_h$ ID but pass the looser alternative $\tau_h$ identification requirement.
...
The raw $\FFi$ take into account dependencies on $N_{\text{jets}}$ via an analysis-category tailed variable $N_{\text{pre b jets}}$, the $\pT$ of the $\tauh$ candidate ($\pT^{\tauh}$) and the $\pT$ of the jet that seeds the \ac{HPS} algorithm ($\pT^{\text{jet}}$).
...
In the $\tauhtauh$ channel, the measured $\FF^{\text{QCD}}$ are then corrected to account for non-closures in other variables in the \texttt{Determination Region}. 
...
To correct for this effect, $\FF^{\text{QCD}}$ are corrected as a function of $C_{\text{QCD}}$, where $C_{\text{QCD}}$ is defined as,
\begin{equation}
...
These fractions are then multiplied by the relevant corrected fake factor and applied to the fail region in C, where this region is purified by subtracting any non \jtth events with \ac{MC}. \\
...
The magnitude and resolution of the missing transverse energy need correcting in embedding events, to take into account the incomplete removal of energy deposits from muons replaced by simulated $\tau$ decays during the embedding procedure. 
...
Uncertainty shifts are applied for the energy scale of leptons faking $\tauh$ parametrised by the $\pT$ of the e/$\mu\rightarrow\tauh$ fake.
...
For the $M_{h,EFT}^{125}$ scenario, the sensitivity sharply drops at $m_{A}=2m_{t}$ due to a drop in the branching fractions for the decay of A and H into $\tau$ leptons, when the A and H decay into two on-shell top quarks becomes kinematically accessible. 
