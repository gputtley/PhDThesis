...
Therefore, corrections on the reconstruction and identification efficiencies are performed in muon $\eta$ and $\pT$ using the \say{tag-and-probe} method. \\
...
These fraction of events are estimated with \ac{MC}, with a \ac{QCD} model extrapolated from same sign $\tau$ pairs.
...
These fractions are then multiplied to the relevant corrected fake factor and applied to the fail region in C, with any events which are not jet$\rightarrow\tau_h$ subtracted off with MC. \\
...
For the $\tauhtauh$ channel there are two $\tau_h$ candidates that a jet can be misidentified as.
For this analysis, the $\FF$ are only applied the leading $\tau_h$ candidate failing the $\tau_h$ ID in C.
...
A \ac{MC} to data correction for a b jet to pass the selection criteria is also determined. 
...
Further \ac{MC} to data corrections are applied to the dilepton mass and $p_{T}$ spectra in simulated $Z\rightarrow ll$ events. 
...
The statistical uncertainties are taken into account by the Barlow-Beeston method, described in Reference~\cite{Barlow:1993dm,Conway:2011in}.
