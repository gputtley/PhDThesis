...
\begin{enumerate}[i)]
  \item A model independent search for single narrow spin-0 resonance ($\phi$), produced via gluon fusion (gg$\phi$) or in association with a b quark (bb$\phi$). The \ac{SM} Higgs boson is treated as a background and the Yukawa couplings of the spin-0 resonance that contribute to the gluon fusion loop are set to \ac{SM} values.
   \item A search for the \ac{MSSM} Higgs sector in a number of benchmark scenarios. The benchmark scenarios were proposed in References~\cite{Bahl:2018zmf,Bahl:2020kwe,Bahl:2019ago} and described fully in Reference~\cite{Bagnaschi:2791954}. The $M_{h}^{125}$ and $M_{h,\text{ EFT}}^{125}$ scenarios are shown in this chapter. The production of the observed Higgs boson particle at 125 GeV is also used to constrain the available phase space.
...
Examples of the Feynman diagrams for such processes are shown in Figure~\ref{fig:mssm_feynamn}, where in this chapter $\phi$ is used to represent the $\Ph$, $\PH$, and $\PA$ bosons.\\
...
$\tau$ leptons are identified with a higher purity than b quarks at the \ac{CMS} detector.
...
where $Q_i$ are resummation scales that depend on the mass of the additional Higgs boson, and $\sigma^{i}_{j}$ and $Y^{i}_{j}$ are the determined cross sections and Yukawa couplings of the relevant theory $j$.
...
Production in association with bottom quarks is simulated at \ac{NLO} precision using the corresponding \POWHEG 2.0 implementation in the four-flavour scheme~\cite{Nason:2004rx,Frixione:2007vw,Alioli:2010xd,Jezo:2015aia}.
...
The differences lie in scaling of the gluon fusion cross section and loop contributions (set to the Yukawa couplings for the model independent interpretation), and the b associated production cross section, as well as taking into account the branching fractions in the model dependent interpretation.
The model dependent search for the \ac{MSSM} also looks to find differences from the observed \ac{SM} Higgs boson and the predicted \ac{MSSM} \ac{SM}-like Higgs boson.
...
\end{subfigure}
\vspace*{10mm}
\caption{Feynman diagrams showing the contribution from $U_1$ vector leptoquarks to the final state with a pair of oppositely charged $\tau$ leptons. $q$ represents any SM quark. Diagram (a) shows t-channel, (b) pair and (c) single production of a vector leptoquark.}
...
A search for single and pair production at the \ac{CMS} experiment was performed and no statistically significance derivation was observed~\cite{CMS:2022zks}.
...
Although there are no additional b quarks in the final state in the \ac{LO} process, initial state radiation can lead to additional b quarks in the final state.
...
The signal process of the $U_1$ t-channel exchange is simulated in the five \ac{FS} at \ac{LO} precision using the \MGvATNLO v2.6.5 event generator~\cite{Alwall:2011uj}.
...
The interference samples are then generated with extra statistics at large di-$\tau$ mass values, in order to have a sufficient number of events in the regions of interest for the signal.
...
In these four final state pairs, a number of different online trigger requirements are needed.
...
The $\pT$ thresholds of light lepton in these triggers are shown in Table~\ref{tab:trig_thresholds}
The cross-trigger is used for events where the light lepton has $\pT$ between the thresholds for the cross-trigger and single-$e/\mu$.
...
All offline selections stated are in addition the object selection discussed in Chapter~\ref{sec:object_reconstruction}.
...
The looser selections vetoed, help to suppress the $Z \rightarrow ll$ background process further, as well as keeping the channels orthogonal. \\
...
The variable is used to remove W + jets background events, where a jet is misidentified as a $\tau_h$, and the \ac{MET} (neutrino) and light lepton from the W decay are aligned and hence the event has a large $m_{T}(\pTvec^{\hspace{2pt}e/\mu},\pTvec^{\hspace{2pt}\text{MET}})$. \\
...
No b tag events failing this cut are vetoed, and events with a b tag also failing this cut are used for a $\ttbar$ control region and discussed further in Section~\ref{sec:background_modelling}.

\section{Search optimisation}
\label{sec:search_optimisation}

The optimisation of the signal extraction depends on which of the three scenarios, set out at beginning of this section, is being searched for.
...
The $\emu$ channel, is subdivided into three signal categories based of the cuts on the variable $D_{\zeta}$ as defined in Equation~\ref{eqn:Dzeta_def}.
...
The relevant backgrounds for this analysis include Drell-Yan, $\ttbar$, W+jets, QCD, di-boson, single-top, and electroweak W and Z bosons production.
...
\begin{enumerate}[i)]
  ...
  \begin{itemize}
    ...
    \item Events with a light lepton misidentified as a $\tau_h$ and no jet misidentified as a $\tau_h$. 
...
Group (ii) is dominated by \ac{QCD}, W + jets and $\ttbar$ events with a jet$\rightarrow \tauh$ misidentification.
This is modelled from data by the fake factor method ($\FF$)~\cite{Sirunyan:2018zut,CMS:2018lkr} and is explained in Section~\ref{sec:ff}.
...
The $\ttbar$ process from (iv) is separated because of to its large contribution to the phase space where a b jet is required. \\

In 2016 (2017--2018), the W + jets and $Z\rightarrow ll$ processes are simulated using the \MGvATNLO 2.2.2 (2.4.2) event generator at \ac{LO}~\cite{Alwall:2011uj}. 
...
For diboson production, \MGvATNLO is used at \ac{NLO} precision~\cite{Alwall:2011uj}, and the FxFx~\cite{Frederix:2012ps} (MLM~\cite{Alwall:2007fs}) prescription is used to match the \ac{NLO} (\ac{LO}) matrix element calculation with the parton shower model. 
...
When compared with data, W + jets, $Z\rightarrow ll$, $\ttbar$, and single top quark events in the tW channel are normalised to their cross sections at \ac{NNLO} precision\cite{Melnikov:2006kv,Czakon:2011xx,Kidonakis:2013zqa}, while single top quark (t-channel) and di-boson events are normalized to their cross sections at \ac{NLO} precision or higher~\cite{Kidonakis:2013zqa,Campbell:2011bn,Gehrmann:2014fva}.

\section{QCD estimation in the $\emu$ channel}
\label{sec:qcd}

The \ac{QCD} model in the $\emu$ channel, that attempts to model events where two jets are misidentified as an electron muon pair, is taken from data with a same sign pair with a transfer factor ($F_{T}$).
...
$F_{T}$ is initially parametrised by the $\Delta R$ between the election and muon, and the number of jets in the event, however additional dependencies on the electron and muon $\pT$ enter via a correction. \\
...
Once completed, the selected muons and their kinematic properties are replaced with a ssimulated $\tau$ lepton.
To account for the difference in mass between the muon and $\tau$, the muons are boosted into the center-of-mass frame of the di-muon system and then this four-vector is taken for the $\tau$ but boosted back into the laboratory frame.
...
Backgrounds in which a jet is misidentified as a $\tauh$ can be difficult to model using \ac{MC} due to the poor description of the $\text{jet}\rightarrow\tauh$ fake rate in simulation. 
...
Firstly, the determination regions are defined, which are $\text{jet}\rightarrow\tauh$ enriched control regions orthogonal to the signal region. 
They are then purified by subtracting off any non $\text{jet}\rightarrow\tauh$ events with \ac{MC}.
It is used to calculate $\FF$ by taking the ratio of number of jet$\rightarrow\tauh$ events that pass the nominal $\tau_h$ ID requirement ($N(\texttt{Nominal})$), to the number of jet fake events that fail the nominal $\tau_h$ identification but pass a looser alternative $\tau_h$ identification requirement ($N(\texttt{Alternative}~\&\&~\texttt{!Nominal})$), as shown in Equation~\ref{eqn:ff}.
...
This is defined as the signal region but with the criteria that the jet$\rightarrow\tauh$ events fail the nominal $\tau_h$ ID but pass the looser alternative $\tau_h$ identification requirement.
This now models the background from $\text{jet}\rightarrow\tauh$ events in signal region. \\
...
The $\FF$ are measured separately in each year of data taking period (2016, 2017, 2018), in each channel containing a $\tau_h$ ($\etauh$, $\mutauh$, $\tauhtauh$) and in enriched regions of dominant processes that contribute $\text{jet}\rightarrow\tauh$ events.
...
Section~\ref{sec:ff_applying} discusses how a single jet misidentified as a $\tauh$ candidate events in the $\tauhtauh$ channel are modelled. \\
...
Region B is an alternative region used to measure and fit the $\FF$ to account for the difference in fake factors between A and C.
...
In the $\mu\tauh$ and $e\tauh$ channels, W + jets jet$\rightarrow\tauh$ events are in general the most significant and \ac{QCD} contributes a smaller fraction. 
...
It is not possible to define a \texttt{Determination Region} that is sufficiently pure in $\ttbar$ events to make a reasonable measurement of $\FF^{\ttbar}$ from data.
Therefore $\FF^{\ttbar}$ are derived from \ac{MC}. 
A comparison of the $\FF^{\text{W+jets}}$ measured in data and \ac{MC} shows only $\sim$ 10--20\% differences in the fake rates in data and \ac{MC}.  
This observation coupled with the fact that the $\ttbar$ contribution is small compared to the other processes means that any bias introduced by using $\FF^{\ttbar}$ measured in \ac{MC} is small compared to the uncertainties on the $\FF$, discussed in Section~\ref{sec:uncerts}. 
In many cases, selections on the regions B and D are applied which are tighter than required, in order to purify the regions as much as possible, while maintaining enough statistics to perform the fit.\\


\subsection{Parametrisation}
\label{sec:ff_params}

The raw $\FFi$ take into account dependencies on $N_{\text{jets}}$ via an analysis category tailed variable $N_{\text{pre b jets}}$, the $\pT$ of the $\tauh$ candidate ($\pT^{\tauh}$) and the $\pT$ of the jet that seeds the \ac{HPS} algorithm to the $\tauh$ ($\pT^{\text{jet}}$).
...
It is defined to map the dependence of $\FFi$ on the number of jets and describe the categorising variable $N_{\text{b jets}}$ well.
...
\caption{A 2D heat map of the $\FF$ determined from $t\bar{t}$ MC for the full Run 2 dataset in the combined $e\tauh$ and $\mu\tauh$ channels. This is shown with respect to the $\tau_h$ $\pT$ and the $\pT$ of the jet matched to the $\tau_h$. The ratio of jet to $\tau_h$ $\pT$ categorisation used is shown split by the dashed lines.}
\label{fig:ff_colz}
\end{figure}
...
When $\pT^{\text{jet}}$ is less than $\pT^{\tauh}$, charge pions are likely not close enough to the \ac{PV} to be clustered into the jet and so the event is less likely to be classified as a hadronic tau. 
...
The fits are flattened at $\pT^{\tau_h}$ values where there is no significant downwards shift or at the final bin to avoid extrapolating past the fitted range. 
...
\caption{$\FF$ fits in $\tauh\tauh$ channel for the QCD $N_{\text{pre b jets}}=0$ category with 2018 data. The three jet $\pT$ to $\tau_h$ $\pT$ categories are shown.}
\label{fig:tt_ff_fit}
\end{figure}
...
To correct for this effect, the \ac{QCD} fake factors are corrected as a function of $C_{\text{QCD}}$, where $C_{\text{QCD}}$ is defined as,
\begin{equation}
...
\label{eqn:c_w}
\end{equation}
When either correction variable is non 0, a larger quantity of fake \ac{MET} is expected in the event. 
...
\begin{figure}[!hbtp]
\centering
    \subfloat[]{\includegraphics[width=0.33\textwidth]{Figures/ff_closure_ss_closure_nbjet0_qcd_tt_2018_v2.pdf}} 
    \subfloat[]{\includegraphics[width=0.33\textwidth]{Figures/ff_closure_met_0jet_closure_qcd_mt_2018_e2.pdf}}
...
\begin{figure}[!hbtp]
\centering
    \subfloat[]{\includegraphics[width=0.33\textwidth]{Figures/ff_closure_os_closure_nbjet0_alt_qcd_tt_2018_v2.pdf}} 
...
\end{equation}
which is the fraction of events with a $\text{jet}\rightarrow\tauh$ originating from process $i$ over the total number of $\text{jet}\rightarrow\tauh$ events for all processes in the \texttt{Application Region}.
These fraction of events are estimated with \ac{MC}, with a \ac{QCD} model extrapolated from same sign $\tau$ pairs,and examples are shown in Figure~\ref{fig:mt_ff_frac}.
It is observed that W + jets is the dominating process in this region, however, there are affects from \ac{QCD} at low $\mT$ and from $\ttbar$ in the b tagged categories. 
These fractions are then multiplied by the relevant corrected fake factor and applied to the fail region in C, where this region is purified by subtracting any non jet$\rightarrow\tau_h$ events with \ac{MC}. \\
...
\caption{The expected \texttt{Application Region} fractions of the processes in the $\mutauh$ channel. (a) and (b) show the \texttt{no b tag Tight-$m_{T}$} and \texttt{Loose-$m_{T}$} categories and (c) and (d) show the \texttt{b tag Tight-$m_{T}$} and \texttt{Loose-$m_{T}$} categories respectively.}
\label{fig:mt_ff_frac}
\end{figure}

The $\tauhtauh$ channel has two $\tau_h$ candidates that a jet can be misidentified as.
For this analysis, the $\FF$ are only applied the leading $\tau_h$ candidate failing the $\tau_h$ identification in C.
...
The magnitude and resolution of the missing transverse energy needs correcting in embedding events, to take into account the incomplete removal of energy deposits from muons replaced by simulated $\tau$ decays during the embedding procedure. 
These corrections are derived by comparing $p_{T}^{\text{miss}}$ in embedded events to fully simulated events. \\

In fully simulated events, a specific trigger inefficiency caused by a shift in the timing of the inputs of the \ac{ECAL} L1 trigger in the region at $|\eta|>2.5$ during the 2016 and 2017 data taking~\cite{CMS:2020cmk}, is needed to be corrected for.
...
This corrections can alter the number of b jets in a simulated event. \\
...
Further \ac{MC} to data corrections are applied to the di-lepton mass and $\pT$ spectra in simulated $Z\rightarrow ll$ events. 
These are derived from $Z\rightarrow\mu\mu$ events.
Additionally, all simulated $\ttbar$ events are weighted to match the top quark $\pT$ distribution observed in data~\cite{CMS:2015rld}.
...
The embedded samples use the same uncertainty scheme as \ac{MC} but 50\% are correlated and 50\% uncorrelated with MC uncertainties, because of the shared real data in the measurement.
...
Background with jets misidentified as electrons or muons from \ac{QCD} are only considered in the $\emu$ channel and modelled from data.
...
These events are modelled from \ac{MC} and any generator matched muon identified as an electron are given a 15\%--45\% uncertainty, that is derived from the initial corrections.

\subsubsection{MET}
The \ac{MET} uncertainties is different dependent on process.
...
Uncertainties on the $\ttbar$ $\pT$ and Z boson $m_{ll}$-$\pT$ reweighting is placed by the applying the correction twice and not at all.
...
For the gg$\phi$ and bb$\phi$ processes, the variation of \texttt{hdamp} parameter of the \POWHEG MC generator as well as the $\mu_{R}$/$\mu_{F}$ scale variations are used to determine the uncertainties on the $\pT$ spectrum of each contribution at NLO QCD to the Higgs boson production via gluon fusion (top, bottom, top-bottom interference). 
These are also determine from additional samples produced up to the generator level, and applied as event weights dependent on generator level $\pT$ after the parton shower simulation.
For the vector leptoquark signal samples, the PDF and $\mu_{R}$/$\mu_{F}$ scale variations are applied on an event-by-event basis.
...
The vector leptoquark search has two signal modes; the t-channel interaction and the interference with Drell-Yan. 
...
The gg$\phi$ samples are also split into the separate loop contributors, so the kinematic properties can be properly scaled to \ac{MSSM} prediction, as described in Section~\ref{sec:additional_higgs_bosons}.
...
Upper limits are placed using the modified frequentest approach \cite{Junk:1999kv,Read:2002hq} with a profile likelihood ratio used for the test statistic, as defined below.
\begin{equation}
...
95\% \ac{CL} limits are set on the assumption of absence of a signal for the search for a gg$\phi$ or bb$\phi$ resonance and shown in Figure~\ref{fig:model_independent_limits}.
...
A comparison of the limits are also made with the ATLAS experiment and in particular the results presented in Reference~\cite{ATLAS:2020zms}.
...
The expected limits from the \ac{CMS} and ATLAS results are roughly compatible over the shared mass range, except at high mass where the extra statistics from the embedded $Z\rightarrow \tau\tau$ samples compared to \ac{MC} allow for lower background uncertainties and hence a stronger limit.
...
The ATLAS search does not stretch to the mass of the low mass \ac{CMS} excess and so cannot be used a cross-check for this.
...
\caption{Comparison of the expected 95\% CL upper limits on the product of the cross sections and branching fraction for the decay into $\tau$ leptons for (a) gg$\phi$ and (b) bb$\phi$ production, split by the CMS result detailed in this thesis and the ATLAS result from Reference~\cite{ATLAS:2020zms}.}
...
\caption{Local $p$-value and significance of a gg$\phi$ (left) and bb$\phi$ (right) signal as a function of $m_{\phi}$~\cite{CMS:2022rbd}.}
...
Other \ac{MSSM} scenarios are tested and detailed in \cite{CMS:2022rbd}.
...
\caption{Expected and observed 95\% CL exclusion contours in the MSSM $M_{h}^{125}$ (a) and $M_{h,EFT}^{125}$ (b) scenarios. The exclusion limit only on background expectation is shown as a dashed black line, the dark and bright grey bands show the 68\% and 95\% intervals of the expected exclusion and the observed exclusion contour is shown by the blue area. The parameter space where $m_{h}$ deviates by more then $\pm$3  GeV from the observed SM Higgs boson mass it shown by a red hatched area.~\cite{CMS:2022rbd}
}
\label{fig:mssm_limits}
\end{figure}

Upper limits of 95\% \ac{CL} for VLQ BM 1 and 2 are shown Figure~\ref{fig:vlq_limits}. 
